...
// media: [{img,label,slug}] — 1 элемент для brand/note, 2 для price
...
// Порог популярности — берём только узнаваемые ароматы (≥300 оценок + есть фото).
const POP_MIN = 300;
...
export async function generateQuestions(): Promise<QuizQuestion[]> {
  const pool = await prisma.$queryRawUnsafe<PoolRow[]>(
    `SELECT f.id, f.name, f.slug, f."imageUrl" AS img, b.name AS brand,
     ...
     WHERE f."imageUrl" IS NOT NULL AND f."popularity" >= ${POP_MIN}
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const fakes = shuffle(POPULAR_NOTES.filter((n) => !own.has(n.toLowerCase()))).slice(0, 3);
